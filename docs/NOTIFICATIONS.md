...
- `data/company_metadata.json` owns canonical aliases, parent companies, featured designations, and recommendation presets.
...
4. Set these Supabase function secrets:
   ...
   - `PUBLIC_SITE_URL`, the HTTPS URL of `docs/notifications/`
...
5. Set `apiUrl` in `docs/notifications/config.js` to the deployed `subscription-api` URL. If Turnstile is enabled, also set `turnstileSiteKey`; the form loads and submits the widget automatically.
